...
| `--min-benefit` | - | 否 | 10.0 | CRF采样预估的最小收益阈值（百分比），低于此值跳过转码 |
...
```
使用 CRF 23 进行恒定质量编码。程序会自动进行多点采样预估，如果压缩收益低于 10% 则跳过转码。
...
# 保守模式（确保质量）
python main.py -i ./videos --crf 20 --min-benefit 5

# 激进模式（追求体积）
python main.py -i ./videos --crf 28 --min-benefit 15
...
### Q: 为什么有些视频被跳过了（CRF模式）？
A: CRF 模式下，如果采样预估显示压缩收益低于设定的阈值（默认10%），说明视频已经是高质量或已高度压缩，继续转码收益很低，因此自动跳过。
